...
function useDarkMode({ isFakeDark }) {
...
function Button({ children }) {
    const { setIsFakeDark } = useContext(PostContext)

    function handleFakeDark() {
        setIsFakeDark((isFakeDark) => !isFakeDark)
    }
    return (
        <button onClick={handleFakeDark}
            className="btn-fake-dark-mode">{children}</button>
    )
}
